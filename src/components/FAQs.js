...
const FAQs = () => {
  ...
  return (
    <div className="container mx-auto mt-8">
      <h1 className="text-4xl font-bold mb-4" style={{color:'#0F6FFF', fontSize:48}}>Frequently Asked Questions (FAQs)</h1><br/>

      {faqs.map((faq, index) => (
        <div
          key={index}
          className={`bg-white p-6 mb-4 rounded-md shadow cursor-pointer ${
            expandedQuestion === index ? 'border border-0F6FFF' : ''
          }`}
        ...
        >
          ...
          {expandedQuestion === index && (
            <div className="mt-4 text-gray-700 font-bold">{faq.answer}</div>
          )}
        </div>
      ))}
    </div>
